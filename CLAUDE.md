...
- **배치 에디터 확장** (PlacedObjectActor ObjectId 저장/하이라이트, PlacementPreviewActor 회전 축 전환/비동기 메시 로드)
- **FPlacedObjectSaveEntry CustomProperties** (TMap<FString, FString> 커스텀 프로퍼티 필드, JumpMap CheckpointOrder 등 저장)
- **JumpMap 배치 CheckpointOrder 자동 할당** (ConfirmPlacement에서 기존 최대 Order+1 자동 부여, TickComponent 3D 텍스트 표시)
- **JumpMap CSV 11번째 Properties 컬럼** (ExportJumpMapLayoutAsCSV에서 Key=Value|Key=Value 형식 직렬화, ParseLayoutCSV와 왕복 호환)
- **JumpMap 레이아웃 검증** (ValidateJumpMapLayout: 체크포인트 최소 1개 + 도착점 정확히 1개, JumpMapEditor ExecuteSave 검증)
- **GameRuleJumpMap CSV 레이아웃 로딩** (MapOption 기반 JumpMapLayoutDataAsset 조회 → SpawnActorsFromLayout → ApplySerializedProperties)
- **대기실 JumpMap 유저 레이아웃 선택** (WaitingRoomHUDWidget: ScanUserJumpMapLayouts → [User] 콤보박스 옵션)
...
[JumpMap Editor → 게임플레이 연동]
유저 CSV 저장 (Content/JumpMapLayouts/User/) + CustomProperties 11번째 컬럼
    ↓
JumpMapLayoutDataAsset.ScanUserJumpMapLayouts() (런타임 스캔)
    ↓
WaitingRoomHUDWidget: [User] 레이아웃 콤보박스 옵션 표시
    ↓
GameModeWaitingRoom.StartGame() → ServerTravel(?MapOption=User_xxx)
    ↓
GameRuleJumpMap.LoadLayoutAndSpawnActors() → GetLayoutByNameIncludingUser()
    ↓
SpawnActorsFromLayout() → 액터 스폰 + ApplySerializedProperties (CheckpointOrder 등)
